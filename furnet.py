...
class Furnet(threading.Thread):
	...
	def gaze(self, x, y, z, head_movement=True):
		...
		head_move = "True" if head_movement else "False"
...
class Demo():
	...
	def execute(self):
		...
		if self.furnet.connected:
                        ...
                        #Sadness
			self.furnet.say("Why?")
			self.furnet.gaze(0.0, -1.0, 0.0, True)
			self.furnet.gesture('emotion_sad')
			self.furnet.gesture('shake')

			#fear, no gaze at all in this
			self.furnet.gesture('emotion_fear')

			#Happiness
			self.furnet.gesture('smile')
			self.furnet.gesture('smile_on')

			#Disgust
			self.furnet.say("Eeeww?")
			self.furnet.gaze(1.0, 1.0, 1.0, True)
			self.furnet.gesture('emotion_disgust')
